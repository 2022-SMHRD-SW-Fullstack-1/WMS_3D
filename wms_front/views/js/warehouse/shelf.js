...
for (let i = 0; i < storage_stock_info.length; i++) {
  ...
  stock_arr.push({
    shelf_num: Number(arr[0]),
    stock_num: Number(arr[1]),
    stock_name: arr[2],
    stock_info: arr[3],
    buy_com: arr[4],
    input_date: arr[5],
    stock_floor: Number(arr[6]),
    stock_position: Number(arr[7]),
  });
}
// 재고 정보 가져오기
...
function changeSub(e) {
  ...
  for (let i = 0; i < stock_arr.length; i++) {
    if (e.shelf_num == stock_arr[i].shelf_num) {
      floor_position_arr.push({
        stock_num: stock_arr[i].stock_num,
        floor: stock_arr[i].stock_floor,
        position: stock_arr[i].stock_position,
      });
    }
  }
  ...
  for (let i = floor; i > 0; i--) {
    ...
    for (let j = length - 1; j >= 0; j--) {
      ...
      for (let k = 0; k < floor_position_arr.length; k++) {
        if (
          floor_position_arr[k].floor == i &&
          floor_position_arr[k].position == j
        ) {
          tf = true;
