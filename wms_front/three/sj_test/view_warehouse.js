...
class App {
			...
			if (found.length > 0) {					// 레이저가 물체를 통과했다면
				...
				if (move_yn != true) {
					...
					if (clickedObj.geometry.type == "BoxGeometry") {
						...
						document.getElementById("stock_num").innerText =
							clickedObj.name.stock_num;
						document.getElementById("stock_name").innerText =
							clickedObj.name.stock_name;
						document.getElementById("stock_info").innerText =
							clickedObj.name.stock_info;
						document.getElementById("input_date").innerText =
							clickedObj.name.input_date;
						document.getElementById("exp_dt").innerText =
							clickedObj.name.exp_dt;
						// clickedObj.parent
					...
					// if(move_yn == true){
					if (
						clickedObj.geometry.type == "BoxGeometry" &&
						clickedObj.name != "shelf_ground"
					) {
						...
						if (oldSelectedMesh !== this._raycaster._selectedMesh) {
							// 현재 선택된 재고에 선 추가
							gsap.to(this._raycaster._selectedMesh.position, {
								y: 7,
								duration: 1,
							});
						...
						if (this._raycaster._selectedMesh) {
							...
							if (found[i].object.parent.name.rotation == "y") {
								move_stock_info.push({
									st_num: move_stock_num,
									shelf_num: found[i].object.parent.name.num,
									st_position: Math.abs(
										Math.floor(
											found[i].point.x - found[i].object.parent.name.x - 1.5
										)
									),
									st_floor: Math.floor(found[i].point.y) + 1,
								});
								// console.log(found)
								...
								move_stock_info.push({
									...
									st_position: Math.abs(
										Math.floor(
											found[i].point.z - found[i].object.parent.name.z - 1.5
										)
									),
									st_floor: Math.floor(found[i].point.y) + 1,
								});
							...
							// 동시에 회전
							gsap.to(this._raycaster._selectedMesh.rotation, {
								y: -Math.PI * 2,
								duration: 1,
							});
							this._raycaster._selectedMesh = null;
	...
	_bringStocks() {
		for (let i = 0; i < stock_num_arr.length; i++) {
			this._bringStock({
				shelf_name: st_shelf_name_arr[i],
				shelf_x: st_shelf_x_arr[i],
				shelf_z: st_shelf_z_arr[i],
				st_shelf_length: st_shelf_length_arr[i],
				shelf_rotation: st_shelf_rotation_arr[i],
				shelf_length: shelf_length_arr[i],
				stock_num: stock_num_arr[i],
				stock_name: stock_name_arr[i],
				stock_info: stock_info_arr[i],
				stock_floor: stock_floor_arr[i],
				stock_position: stock_position_arr[i],
				input_date: input_date_arr[i],
				exp_dt: exp_dt_arr[i],
			});
		}
	}
	_bringStock(i) {
		...
		if (i.shelf_rotation == "y") {
			stock_x =
				Number(i.shelf_x) -
				Number(i.stock_position) +
				(Number(i.st_shelf_length) / 2 - 0.5);
		...
		StockMesh.name = {
			shelf_name: i.shelf_name,
			shelf_x: i.shelf_x,
			shelf_z: i.shelf_z,
			shelf_length: i.shelf_length,
			shelf_rotation: i.shelf_rotation,
			stock_num: i.stock_num,
			stock_name: i.stock_name,
			stock_info: i.stock_info,
			stock_position: i.stock_position,
			stock_floor: i.stock_floor,
			input_date: i.input_date,
			exp_dt: i.exp_dt,
		};
	...
	_bringShelves() {
		for (let i = 0; i < shelf_name_arr.length; i++) {
			this._bringShelf({
				num: shelf_num_arr[i],
				name: shelf_name_arr[i],
				x: shelf_x_arr[i],
				z: shelf_z_arr[i],
				width: shelf_width_arr[i],
				length: shelf_length_arr[i],
				floor: shelf_floor_arr[i],
				rotation: shelf_rotation_arr[i],
			});
		}
	}
	_bringShelf(item) {
					...
					if (intersect.object.name === "shelf_ground") {
						if (i == 0) {
							const highlightPos = new THREE.Vector3()
								...
								.floor()
								.addScalar(0.5);
							const highlightY = new THREE.Vector3()
								.copy(intersect.point)
								.floor();
